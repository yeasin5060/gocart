                ...
                line_items : [{
                    price_data: {
                        currency: 'usd',
                            ...
                            unit_amount: Number(Math.round(fullAmount * 100))
                        },
                    quantity : 1
                }],
                expires_at : Math.floor(Date.now()/1000) + 30 * 60, // current time + 30 minites
                mode : 'payment',
...
export async function GET(request) {
    try {
        ...
        const orders = await prisma.order.findMany({
         ...
         return NextResponse.json({orders,message : 'order create successfully'});
    } catch (error) {
        console.error(error);
        return NextResponse.json({message : error.message},{status:400});
    }
